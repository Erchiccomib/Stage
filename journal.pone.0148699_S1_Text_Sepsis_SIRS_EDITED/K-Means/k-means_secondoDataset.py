...
features_ = dataset.drop(columns=['site', 'time_months', 'outcome'])
...
plt.ylabel('Inertia')
plt.title('Gomito (Tramite StandardScaler)')
plt.grid(True)
plt.show()
...
plt.title('K-means (k=4 con StandardScaler)')
